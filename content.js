...
function initialization () {
  chrome.storage.sync.get(['shortcuts', 'snapTextEnabled'], (data) => {
    snapTextEnabled = data.snapTextEnabled ?? true;
...
//DUE TO THE WHOLE INITIALIZATION CODE ABOVE, WE COMMENTED OUT THE CODE BELOW!!!
...
//   if (!snapTextEnabled) {
//     console.log("SnapText is disabled");
//     return;
//   }
//   console.log("script loaded");

//   loadShortcuts(); // this will cause bug, as loadshortcut will run but code wont wait for all of them to get loaded, causing error.as event listeners of shortcuts are attached instantly so ofc, expandshortcut wont work w/o the shortcut loaded.
...
// // if(!snapTextEnabled) return; // to make sure we act upon the value that we are getting from popup.js.
...
function expandShortcuts(event) {
  if(!snapTextEnabled) return; // this line makes sure that if my snaptext is wokring then only execute this function, else dont execute expandshortcut
...
function processInputOrTextArea(element) {
  ...
  console.log("Last word detected:", `'${original}', '${sanitized}'`);

  if (shortcuts[sanitized]) {
    // changes were made lastWord -> sanitized.
    ...
    const { newText, newCursorPosition } = replaceLastWordInText(
      text,
      cursorPosition,
      original,
      shortcuts[sanitized]
...
function isTargetElement(el) {
  // the basic use case of this function is that we will call this fucntion whenever we need to check if the node/element is textarea, input or contenteditable, and it will be returning true or false.
  return (
    // this dosent work on its own, we use it just to check so we call it when we need and it answers accordingly.
...
const observer = new MutationObserver((mutationsList) => {
  // so here this MutationObserver is an object that watches the DOM, and whenever the change is detected like an element is added, it triggers the callback function present inside mutationsList i.e. for list of changes that took place.
  for (const mutation of mutationsList) {
    // this means out of the list provided to it of the changes that is mutations here, take a single mutation, its written here like a enhanced for loop in java.
    if (mutation.type === "childList") {
      // we only want to cater the new elements added so we onky focus on the type "childList".
      mutation.addedNodes.forEach((node) => {
        // for every change/mutation taking place, there can be multiple elements/nodes added for each mutation so we need to sort out the ones we need.
        ...
        if (isTargetElement(node)) {
          // we call the function from top that we created to check. this if condition is checking the "parent node".
        ...
        if (node.nodeType === Node.ELEMENT_NODE && node.querySelectorAll) {
          // this if condition is for the children elements under the parent, for cases like if parent isnt a input, textarea or contenteditable but the children are.
          const fields = node.querySelectorAll(
            "input, textarea, [contenteditable='true']"
          ); // querySelector is required in order to select those 3 things out of the element
          fields.forEach((el) => {
            el.addEventListener("keydown", expandShortcuts);
          });
        }
...
function replaceLastWordInContentEditable(node, offset, lastWord, expansion) {
  ...
  const newText =
    text.substring(0, lastWordStart) + expansion + text.substring(offset);
